...
namespace Physics.ElectricParticle.Logic
{
	public class PhysicsService : IPhysicsService
	{
		...
		public BigDecimal ComplexCosAxisCoordinate(decimal time)
		{
		...
		public BigDecimal ComplexSinAxisCoordinate(decimal time)
		{
		...
		private BigDecimal ComplexAxisCoordinate(
			double time,
			double goniometricAngle,
			PlaneSetup plane)
		{
			var left = _setup.Particle.StartVelocity * goniometricAngle * time;
			var upperFraction = (int)plane.Polarity * plane.Voltage * (int)_setup.Particle.Polarity * GetChargeBase() * (time * time);
		...
		public decimal SimpleCosAxisCoordinate(decimal time) => (decimal)_setup.Particle.StartVelocity * (decimal)Math.Sin(_setup.Particle.StartVelocityDeviation) * time;

		public decimal SimpleSinAxisCoordinate(decimal time) => (decimal)_setup.Particle.StartVelocity * (decimal)Math.Cos(_setup.Particle.StartVelocityDeviation) * time;

		public BigDecimal ComputeX(double time)
		{
			...
			switch (_setup.Variant)
			{
				...
				case InputVariant.AdvancedVerticalWithGravity:
					return ComplexCosAxisCoordinate(time);
		...
		public BigDecimal ComputeY(double time)
		{
			switch (_setup.Variant)
			{
				case InputVariant.EasyVerticalNoGravity:
				...
				case InputVariant.AdvancedVerticalWithGravity:
					return ComplexSinAxisCoordinate(time);
				case InputVariant.EasyHorizontalNoGravity:
				case InputVariant.EasyHorizontalWithGravity:
					return ComplexCosAxisCoordinate(time);
				case InputVariant.AdvancedVerticalHorizontalNoGravity:
