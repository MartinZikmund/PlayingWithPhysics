//TODO: Add czech accent characters to CanHandle and then
// ensure they are handled correctly in BreakWord
...
{
	public class CsBreakingEngine : BreakingEngine
	{
		enum LexState
		{
			Init,
		...
		private char[] _czechAccents = new char[]{
			'á', 'č', 'ď', 'é', 'ě', 'í', 'ň', 'ó', 'ř', 'š', 'ť', 'ú', 'ů', 'ý', 'ž'
		};


		BreakBounds _breakBounds = new BreakBounds();
		public CsBreakingEngine()
		{
		...
		public override bool CanHandle(char c)
		{
			...
			if (_czechAccents.Contains(char.ToLowerInvariant(c)))
			{
				return true;
			}
